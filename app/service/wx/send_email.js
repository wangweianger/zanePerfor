...
class SendEmailService extends Service {
    ...
    async daliyHtmlTem(datas) {
        ...
        if (toplist.topbrand && toplist.topbrand.length) {
            for (let i = 0, len = toplist.topbrand.length; i < len; i++) {
                topbrand += `<div style="display:inline-block;margin-right:20px;margin-bottom:20px;">${toplist.topbrand[i]._id.brand}：<span style="color:#8776f7;">${toplist.topbrand[i].count}</span></div>`;
            }
        }
                ...
                <div>
                    ...
                    <div style="width:90%;margin:0 auto;background:#fff;border-radius:6px;padding:20px;overflow:hidden;">
                        ...
                        <div>
                            ...
                            <div style="display: inline-block;margin-right:20px;font-size:18px;padding:0 20px;">流量：<span style="color:#8776f7;">${this.flow(pvuvip.flow)}</span></div>
                        </div>
                    </div>
